...
const KelolaKategori = () => {
  ...
  const [incomeCategories, setIncomeCategories] = useState([
    ...
    },
    {
      ...
      sublist: [{ name: "Gaji" }, { name: "Part time job" }],
    },
    {
      name: "Lain (Pemasukan)",
      icon: AddIcon,
      sublist: [{ name: "Tabungan pribadi" }],
    },
  ]);
  ...
  const startEditing = (type, categoryIndex, subItemIndex = null) => {
    const editKey = `${type}-${categoryIndex}${
      subItemIndex !== null ? `-${subItemIndex}` : ""
    }`;
    const categories =
  ...
  const deleteItem = (type, categoryIndex, subItemIndex = null) => {
    if (type === "pemasukan") {
      if (subItemIndex !== null) {
        // Delete subitem
        const newCategories = [...incomeCategories];
        newCategories[categoryIndex].sublist = newCategories[
          categoryIndex
        ].sublist.filter((_, i) => i !== subItemIndex);
        setIncomeCategories(newCategories);
      } else {
        // Delete category
        setIncomeCategories(
          incomeCategories.filter((_, i) => i !== categoryIndex)
        );
      }
    } else {
      if (subItemIndex !== null) {
        ...
        setExpenseCategories(
          expenseCategories.filter((_, i) => i !== categoryIndex)
        );
      }
    }
  };
    ...
    if (type === "pemasukan") {
      ...
      setTimeout(() => {
        startEditing(
          type,
          categoryIndex,
          newCategories[categoryIndex].sublist.length - 1
        );
      }, 100);
    } else {
      const newCategories = [...expenseCategories];
              ...
              >
                ...
                <div className="flex items-center">
                  {editingItem === `${type}-${index}-${subIndex}` ? (
                    <>
                      <button
                        onClick={() => saveEdit(type, index, subIndex)}
                        className="text-green-500 mr-2"
                      >
                        <CheckIcon sx={{ fontSize: 18 }} />
                      </button>
                    ...
                    <>
                      <button
                        onClick={() => startEditing(type, index, subIndex)}
                        className="text-blue-500 mr-2"
                      >
                        <EditIcon sx={{ fontSize: 18 }} />
                      </button>
                      <button
                        onClick={() => deleteItem(type, index, subIndex)}
                        className="text-red-500"
                      >
                        <DeleteIcon sx={{ fontSize: 18 }} />
                      </button>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-lg mx-auto bg-white rounded-lg shadow">
        ...
        <div className="p-4">
          {activeTab === "pemasukan" &&
            renderCategories(incomeCategories, "pemasukan")}
          {activeTab === "pengeluaran" &&
            renderCategories(expenseCategories, "pengeluaran")}
        </div>
      </div>
